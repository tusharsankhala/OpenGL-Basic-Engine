...
void SceneBasic::CompileShaderProgram() {
    ...
    std::ifstream inFile("../Shaders/Basic.vert");
    if (!inFile)
    {
    ...
    if (!fragFile)
    {
        fprintf(stderr, "Error opening file: Shaders/basic.frag\n");
        exit(1);
    }
...
void SceneBasic::LoadShaderBinary(GLint format)
{
    ...
    std::vector<char> buffer(startIt, endIt);
    inStream.close();
    glProgramBinary(m_programHandle, format, buffer.data(), buffer.size());
...
void SceneBasic::WriteShaderBinary()
{
    ...
    if (formats > 0) {
        GLint length;
        glGetProgramiv(m_programHandle, GL_PROGRAM_BINARY_LENGTH, &length);
        std::cout << "Program binary length = " << length << std::endl;

        std::vector<GLubyte> buffer(length);
        GLenum format;
        glGetProgramBinary(m_programHandle, length, NULL, &format, buffer.data());
